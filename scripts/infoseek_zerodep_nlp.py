"""
infoseek_zerodep_nlp.py — 零依赖 NLP 原语（v1.0.1）
=====================================================================

⚠️ QCM vendored 副本（2026-08-22 vendored from infoseek/scripts/）
本文件自 `~/.workbuddy/skills/infoseek/scripts/infoseek_zerodep_nlp.py` 复制，
作为 QCM 本地零依赖兜底独立发行——**不依赖 infoseek 是否安装/跨平台部署**。
若上游 infoseek 更新此模块，请同步回拷（保持单一事实源的版本追溯）。
